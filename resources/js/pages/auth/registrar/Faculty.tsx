...
const NAV_ITEMS = [
    ...
    { label: 'Faculty',     href: '/registrar/faculty',     icon: 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z', active: true },
...
export default function Faculty({ faculty, flash, registrar }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent, type: 'add' | 'edit') => {
        e.preventDefault();
        if (!form.employee_id.trim() || !form.first_name.trim() || !form.last_name.trim() || !form.email.trim()) return;
        setSubmitting(true);
        if (type === 'add') {
            router.post('/registrar/faculty', form, {
                onSuccess: () => { setShowAddModal(false); resetForm(); setSubmitting(false); },
                onFinish: () => setSubmitting(false),
                preserveState: true,
            });
        } else if (editingFaculty) {
            router.put(`/registrar/faculty/${editingFaculty.id}`, form, {
                onSuccess: () => { setEditingFaculty(null); resetForm(); setSubmitting(false); },
                onFinish: () => setSubmitting(false),
                preserveState: true,
            });
        }
    ...
    const removeFaculty = (id: number) => {
        if (confirm('Remove this faculty member? This action cannot be undone.')) {
            router.delete(`/registrar/faculty/${id}`, { preserveState: true });
        }
    };

    const resetForm = () => {
        setForm({ employee_id: '', first_name: '', last_name: '', email: '', department: '', position: '', password: '' });
    };
    ...
    const totalSections = faculty.reduce((sum, f) => sum + (f.sections_count || 0), 0);
    const avgSectionsPerFaculty = (totalSections / totalFaculty || 0).toFixed(1);
    ...
    // Sections distribution for bar chart
    const sectionRanges = [
        { label: '0 sections', value: faculty.filter(f => (f.sections_count || 0) === 0).length, color: '#ef4444' },
        { label: '1-2 sections', value: faculty.filter(f => (f.sections_count || 0) >= 1 && (f.sections_count || 0) <= 2).length, color: '#f59e0b' },
        { label: '3-4 sections', value: faculty.filter(f => (f.sections_count || 0) >= 3 && (f.sections_count || 0) <= 4).length, color: '#10b981' },
        { label: '5+ sections', value: faculty.filter(f => (f.sections_count || 0) >= 5).length, color: '#6366f1' },
    ].filter(bar => bar.value > 0);

    // Faculty funnel
    const funnelItems = [
        { label: 'Total Faculty', value: totalFaculty, color: '#6366f1', max: totalFaculty || 1 },
        { label: 'With Sections', value: faculty.filter(f => (f.sections_count || 0) > 0).length, color: '#10b981', max: totalFaculty || 1 },
        { label: 'Total Sections', value: totalSections, color: '#f59e0b', max: Math.max(totalSections, totalFaculty) || 1 },
    ];

    // Get unique departments for filter
    const departments = Array.from(new Set(faculty.map(f => f.department).filter(d => d)));
                ...
                <div className="relative flex-shrink-0">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                        ...
                        <span className="text-[9px] font-semibold text-slate-400 uppercase tracking-wider mt-0.5">
                            {hovered !== null ? segments[hovered].label : 'Faculty'}
                        </span>
                    </div>
                </div>
    ...
    return (
        <>
            <Head title={`Faculty | ${defaultRegistrar.full_name} | GradeHub`} />
                ...
                <div className="flex-1 flex flex-col overflow-hidden">
                    <header className={`px-6 py-3.5 flex items-center justify-between flex-shrink-0 border-b ${dark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-xs text-slate-400">Pages / Faculty</p>
                                <h1 className={`text-base font-bold leading-tight ${dark ? 'text-white' : 'text-slate-900'}`}>Faculty</h1>
                            </div>
                        </div>
                    ...
                    <main className={`flex-1 overflow-y-auto p-6 ${dark ? 'text-white' : 'text-slate-900'}`}>
                        ...
                        <div className={`rounded-2xl p-5 mb-6 flex items-center justify-between overflow-hidden relative ${dark ? 'bg-indigo-900/40 border border-indigo-800/50' : 'bg-indigo-600'}`}>
                            <div className="relative z-10">
                                ...
                                <h2 className="text-2xl font-bold text-white">
                                    Faculty Management, {defaultRegistrar.full_name.split(' ')[0]} 👨‍🏫
                                </h2>
                                <p className={`text-sm mt-0.5 ${dark ? 'text-indigo-300' : 'text-indigo-200'}`}>
                                    Manage faculty accounts, departments, and teaching assignments
                                </p>
                            </div>
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                            {[
                                { label: 'Total Faculty', value: totalFaculty, sub: 'active teachers', gradient: 'from-indigo-500 to-indigo-600', icon: 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z' },
                                { label: 'Departments', value: deptMap.size, sub: 'across institution', gradient: 'from-emerald-500 to-teal-600', icon: 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4' },
                                { label: 'Total Sections', value: totalSections, sub: 'assigned to faculty', gradient: 'from-amber-400 to-orange-500', icon: 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10' },
                            ].map(card => (
                                <div key={card.label}
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-5">
                            ...
                            <div className={`rounded-2xl border p-5 ${dark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                                <div className="flex items-center justify-between mb-4">
                                    <div>
                                        ...
                                        <p className={`text-xs ${dark ? 'text-slate-500' : 'text-slate-400'}`}>Faculty distribution</p>
                                    </div>
                            ...
                            {/* Sections Distribution Bar */}
                            <div className={`rounded-2xl border p-5 ${dark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                                <div className="flex items-center justify-between mb-4">
                                    <div>
                                        <p className={`text-sm font-bold ${dark ? 'text-white' : 'text-slate-900'}`}>Section Load</p>
                                        <p className={`text-xs ${dark ? 'text-slate-500' : 'text-slate-400'}`}>Courses assigned per faculty</p>
                                    </div>
                                ...
                                {sectionRanges.length === 0 ? (
                                    ...
                                    <BarChart bars={sectionRanges} dark={dark} />
                                )}
                            </div>
                            ...
                            <div className={`rounded-2xl border p-5 ${dark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                                <div className="flex items-center justify-between mb-4">
                                    <div>
                                        <p className={`text-sm font-bold ${dark ? 'text-white' : 'text-slate-900'}`}>Faculty Funnel</p>
                            ...
                            <div className={`px-5 py-4 border-b ${dark ? 'border-slate-800' : 'border-slate-100'}`}>
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                    <div className="flex items-center gap-2.5">
                                        ...
                                        <span className={`text-sm font-bold ${dark ? 'text-white' : 'text-slate-900'}`}>Faculty Directory</span>
                                        <span className="bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-400 text-xs font-semibold px-2 py-0.5 rounded-full">{filteredFaculty.length}</span>
                                    </div>
                            ...
                            <div className="p-5">
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {filteredFaculty.map(f => (
                                        <div key={f.id} 
                                            className={`rounded-2xl border transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg ${dark ? 'bg-slate-800/50 border-slate-700 hover:shadow-slate-800/50' : 'bg-white border-slate-200 hover:shadow-slate-200/80'}`}>
                                            <div className="p-5">
                                                <div className="flex items-center justify-between mb-3">
                                                    <div className="flex items-center gap-3">
                                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0 ${dark ? 'bg-indigo-900/40 text-indigo-400' : 'bg-indigo-100 text-indigo-700'}`}>
                                                            {initials(f.full_name)}
                                                        </div>
                                                        <div>
                                                            <p className={`text-sm font-bold ${dark ? 'text-white' : 'text-slate-900'}`}>{f.full_name}</p>
                                                            <p className={`text-xs ${dark ? 'text-slate-400' : 'text-slate-400'}`}>{f.employee_id}</p>
                                                        </div>
                                                    </div>
                                                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${dark ? 'bg-indigo-900/40 text-indigo-400' : 'bg-indigo-50 text-indigo-600'}`}>
                                                        {f.sections_count || 0} sections
                                                    </span>
                                                </div>
                                                <p className={`text-xs ${dark ? 'text-slate-300' : 'text-slate-500'} mb-1 flex items-center gap-1`}>
                                                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                                                    </svg>
                                                    {f.email}
                                                </p>
                                                <p className={`text-xs ${dark ? 'text-slate-400' : 'text-slate-400'} flex items-center gap-2`}>
                                                    <span className={`px-1.5 py-0.5 rounded ${dark ? 'bg-slate-700' : 'bg-slate-100'}`}>{f.department || '—'}</span>
                                                    <span>·</span>
                                                    <span>{f.position || 'Faculty'}</span>
                                                </p>
                                                <div className="mt-4 flex gap-2 pt-2 border-t ${dark ? 'border-slate-700' : 'border-slate-100'}">
                                                    <button onClick={() => openEdit(f)}
                                                        className={`text-xs px-3 py-1.5 rounded-lg font-semibold transition-colors flex-1 ${dark ? 'border border-slate-600 text-slate-300 hover:bg-slate-700' : 'border border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                                                        Edit
                                                    </button>
                                                    <button onClick={() => removeFaculty(f.id)}
                                                        className="text-xs px-3 py-1.5 border border-rose-200 dark:border-rose-800 rounded-lg text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-900/20 font-semibold transition-colors flex-1">
                                                        Remove
                                                    </button>
                                                </div>
                                            </div>
                                ...
                                {filteredFaculty.length === 0 && (
                                    <div className="py-12 text-center">
                                        ...
                                        <p className={`text-sm font-medium ${dark ? 'text-slate-400' : 'text-slate-400'}`}>
                                            {searchTerm || selectedDepartment !== 'all' ? 'No faculty match your filters' : 'No faculty members added yet'}
                                        </p>
            ...
            {(showAddModal || editingFaculty) && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={() => { setShowAddModal(false); setEditingFaculty(null); resetForm(); }} />
                    <div className={`relative rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden ${dark ? 'bg-slate-900 border border-slate-700' : 'bg-white'}`}>
                        <div className={`flex items-center justify-between px-6 py-5 border-b ${dark ? 'border-slate-800' : 'border-slate-100'}`}>
                            ...
                            <button onClick={() => { setShowAddModal(false); setEditingFaculty(null); resetForm(); }}
                        ...
                        <form onSubmit={(e) => handleSubmit(e, editingFaculty ? 'edit' : 'add')} className="px-6 py-5 space-y-4">
                            <div>
                                <label className={`text-xs font-semibold block mb-1 ${dark ? 'text-slate-300' : 'text-slate-600'}`}>Employee ID</label>
                                <input required value={form.employee_id} onChange={e => setForm({...form, employee_id: e.target.value.toUpperCase()})} disabled={!!editingFaculty}
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div>
                                    <label className={`text-xs font-semibold block mb-1 ${dark ? 'text-slate-300' : 'text-slate-600'}`}>Position / Title</label>
                            ...
                            <div className="flex justify-end gap-3 pt-2">
                                <button type="button" onClick={() => { setShowAddModal(false); setEditingFaculty(null); resetForm(); }}
